...
function fetchWeatherData(selectId, containerId) {
  ...
  // apiUrl = anropar api:et för att hämta väderdata för vald stad
  const apiUrl = `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${apiKey}&units=metric`;

// Begär väderdata från OpenWeatherMap API
  fetch(apiUrl)
    .then((response) => response.json()) // Svart från apiet knventeras till json  
    
    .then((data) => { // När datan är konverterad till json får vi väderinformationen
...
function updateWeatherDisplay(containerId, weatherData) {
  ...
  Object.keys(weatherData).forEach((key) => {
    ...
    if (key === "icon" && element) { // Är det en bild som ska visas sätts src-attributet till urln för bilden
        element.src = weatherData[key];
    } else if (element) {
        element.textContent = weatherData[key]; // Elemnen som inte är bilder får texten satt till värdet (te.x temperatur)
    }
});
}